...
@Injectable()
export class ParcelInfoSmallClaimsParcelInformationSalesEffects {
  constructor(
  ...
  public readonly load$ = createEffect(() => {
    return this._actions$.pipe(
      ofType(ParcelInfoSmallClaimsParcelInformationSalesActions.load, ParcelInfoRefreshActions.refresh),
      withLatestFrom(this._parcelInfoSmallClaimsFacade.computedSmallClaimId$),
      switchMap(([, selectedSmallClaimId]) => {
        return this._pasSwaggerApiService.smallClaim_GetSale(selectedSmallClaimId).pipe(
          map((sales) => ParcelInfoSmallClaimsParcelInformationSalesActions.loadSuccess({ sales })),
          catchApiException(ParcelInfoSmallClaimsParcelInformationSalesActions.loadError)
        );
      })
    );
  });
